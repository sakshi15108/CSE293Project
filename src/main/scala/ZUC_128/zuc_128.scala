package ZUC_128

import chisel3._
import chisel3.util._

import scala.collection.mutable.ArrayBuffer


object zuc128_model {
  /*LSFR state registers*/
  var LFSR_S : ArrayBuffer[Int] = (new ArrayBuffer[Int]) ++ Seq.fill(16)(0)

  /* the registers of F */
  var F_R  : ArrayBuffer[Int] = (new ArrayBuffer[Int]) ++ Seq.fill(2)(0)

  /* the outputs of BitReorganization */
  var BRC_X  : ArrayBuffer[Int] = (new ArrayBuffer[Int]) ++ Seq.fill(4)(0)

  /* the s-boxes */
  val S0 = Seq()

  val S1 = Seq()

  val EK_d : Seq[Int]= Seq(0x44D7, 0x26BC, 0x626B, 0x135E, 0x5789, 0x35E2, 0x7135, 0x09AF,
    0x4D78, 0x2F13, 0x6BC4, 0x1AF1, 0x5E26, 0x3C4D, 0x789A, 0x47AC)

  val MASK = 0x7FFFFFFF

  /* c = a + b mod (2^31 – 1) */
  def AddM(a: Int, b: Int): Int = {
    var c = a + b
    (c & MASK) + (c >> 31)
  }

  def MulByPow2(x: Int, k:Int):Int = {
    ((((x) << k) | ((x) >> (31 - k))) & MASK)
  }

  /* LFSR with initialization mode */
  def LFSRWithInitialisationMode(u:Int) = {
    var f: Int = 0
    var v: Int = 0

    f = LFSR_S(0)
    v = MulByPow2(LFSR_S(0), 8);
    f = AddM(f, v);
    v = MulByPow2(LFSR_S(4), 20);
    f = AddM(f, v);
    v = MulByPow2(LFSR_S(10), 21);
    f = AddM(f, v);
    v = MulByPow2(LFSR_S(13), 17);
    f = AddM(f, v);
    v = MulByPow2(LFSR_S(15), 15);
    f = AddM(f, v);
    f = AddM(f, u);

    /* update the state */
    for (i <- 0 until 15) {
      LFSR_S(i) = LFSR_S(i + 1);
    }
    LFSR_S(15) = f
  }

  /* LFSR with work mode */
  def LFSRWithWorkMode() = {
    var f : Int = 0
    var v : Int = 0
    f = LFSR_S(0);
    v = MulByPow2(LFSR_S(0), 8);
    f = AddM(f, v);
    v = MulByPow2(LFSR_S(4), 20);
    f = AddM(f, v);
    v = MulByPow2(LFSR_S(10), 21);
    f = AddM(f, v);
    v = MulByPow2(LFSR_S(13), 17);
    f = AddM(f, v);
    v = MulByPow2(LFSR_S(15), 15);
    f = AddM(f, v);

    /* update the state */
    for(i<- 0 until 15) {
      LFSR_S(i) = LFSR_S(i+1);
    }
    LFSR_S(15) = f;
  }

  /* BitReorganization */
  def BitReorganization() = {
    BRC_X(0) = ((LFSR_S(15) & 0x7FFF8000) << 1) | (LFSR_S(14) & 0xFFFF);
    BRC_X(1) = ((LFSR_S(11) & 0xFFFF) << 16) | (LFSR_S(9) >> 15);
    BRC_X(2) = ((LFSR_S(7) & 0xFFFF) << 16) | (LFSR_S(5) >> 15);
    BRC_X(3) = ((LFSR_S(2) & 0xFFFF) << 16) | (LFSR_S(0) >> 15);
  }

  def ROT(a: Int, k:Int):Int = {
    (a << k) | (a >> (32 - k))
  }

  def L1(x: Int) : Int = {
    x ^ ROT(x, 2) ^ ROT(x, 10) ^ ROT(x, 18) ^ ROT(x, 24);
  }

  def L2(X: Int) : Int = {
    X ^ ROT(X, 8) ^ ROT(X, 14) ^ ROT(X, 22) ^ ROT(X, 30)
  }

  def MAKEU32(a: Byte, b: Byte, c: Byte, d:Byte) : Int = {
    (a.toInt << 24) | (b.toInt << 16)  | (c.toInt << 8) | (d.toInt)
  }

  def F() = {
    var W , W1, W2, u, v: Int = 0

    val MASK1 = 0xFF

    W = (BRC_X(0) ^ F_R(1)) + F_R(2);
    W1 = F_R(1) + BRC_X(1);
    W2 = F_R(2) ^ BRC_X(2);
    u = L1((W1 << 16) | (W2 >> 16));
    v = L2((W2 << 16) | (W1 >> 16));
    F_R(1) = MAKEU32((S0((u >> 24) & MASK1)).toByte, S1((u >> 16) & MASK1).toByte, S0((u >> 8) & MASK1).toByte, S1(u & MASK1).toByte)
    F_R(2) = MAKEU32(S0((v >> 24) & MASK1).toByte, S1((v >> 16) & MASK1).toByte, S0((v >> 8) & MASK1).toByte, S1(v & MASK1).toByte)
    W
  } //FIXME: S0 and S1 are of type Byte. Change them and remove .toByte from this def.

  def MAKEU31(a: Byte, b: Int, c:Byte) = {
    (a.toInt << 23) | (b << 8) | c.toInt
  }

}

class zuc128_model (k: Seq[Byte] , iv: Seq[Byte], pKeystream: ArrayBuffer[Int], KeystreamLen: Int) {

  /* initialize */
  def Initialization(k: Seq[Byte] , iv: Seq[Byte]) = {
    var w, nCount : Int = 0;
    /* expand key */
    for (i <- 0 until 16) {
      zuc128_model.LFSR_S(i) = zuc128_model.MAKEU31(k(i), zuc128_model.EK_d(i), iv(i));
    }
    /* set F_R1 and F_R2 to zero */
    for (i <- 0 until 2) {
      zuc128_model.F_R(i) = 0;
    }
    nCount = 32;
    while (nCount > 0)
    {
      zuc128_model.BitReorganization();
      w = zuc128_model.F();
      zuc128_model.LFSRWithInitialisationMode(w >> 1);
      nCount = nCount -1;
    }
  }

  def GenerateKeystream(pKeystream: ArrayBuffer[Int], KeystreamLen: Int) = {
    var i: Int = 0;
    {
      zuc128_model.BitReorganization();
      zuc128_model.F(); /* discard the output of F */
      zuc128_model.LFSRWithWorkMode();
    }
    for (i <- 0 until KeystreamLen) {
      zuc128_model.BitReorganization();
      pKeystream(i) = zuc128_model.F() ^ zuc128_model.BRC_X(3); //FIXME: pkeystream is input!? then how assign!?
      zuc128_model.LFSRWithWorkMode();
    }
  }
}